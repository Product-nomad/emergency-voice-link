...
1. **First-response latency.** Dispatcher's first audible response < 800ms from end-of-child-utterance. **Now measured (2026-07-30)**, not just ad-hoc: `useEmergencyCall.ts` uses the ElevenLabs SDK's `onVadScore`/`onAudio` callbacks to time from the child's last detected speech to the agent's first audio chunk, reported to GA as `agent_response_latency` (`response_ms`). This is turn-taking latency *within* an already-connected call — almost entirely ElevenLabs agent-pipeline time (STT → LLM → TTS), not something this codebase controls. If the number comes back high, the levers are on the ElevenLabs agent config (model choice, `optimize_streaming_latency`, VAD/turn-taking sensitivity), not in this repo. Distinct from call-*setup* latency (dial → token ready, dial → WebRTC connected), reported separately as `call_latency`.
